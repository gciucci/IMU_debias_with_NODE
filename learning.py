...
def Gyro_train(dataset_train: BaseDataset, dataset_val: BaseDataset, output_dir: str,  integral_method = "euler", device = "cuda", lr = 0.005, weight_decay=1e-6, epoch = 101):
    ...
    def compute_solution_and_loss(dataset, index):
        ...
        model.set_R0(R0)
        _,R_sol= odeint_SO3(model, y0, R0, t_odeint, method=integral_method)
        criterion = SO3log_loss
        loss = 1e6 * criterion(R_sol, X_gt[...,:3,:3])
        return R_sol, loss
    ...
    if model.bias_net_a is not None:
            for param in model.bias_net_a.parameters():
    ...
    for ep in range(epoch):
        ...
        if ep % val_freq == 0:
            ...
            if val_loss_total < best_loss:
                ...
                best_model_state = {
                    'func_bw_model': model.bias_net_w.state_dict(),
    ...
    # Ensure destination exists before saving checkpoints.
...
def Acc_train(
    ...
    # congela bias gyro
    if model.bias_net_w is not None:
        for param in model.bias_net_w.parameters():
            param.requires_grad = False
    ...
    for ep in range(epoch):
        ...
        if ep % val_freq == 0:
            ...
            if val_loss_total < best_loss:
                best_loss = val_loss_total
                best_model_state = {
                    "func_ba_model": model.bias_net_a.state_dict(),
                    "epoch": ep,
                    "best_loss": best_loss,
                }
